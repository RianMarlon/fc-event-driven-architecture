...
func (a *AccountDB) FindByID(id string) (*entity.Account, error) {
	...
	stmt, err := a.DB.Prepare("select a.id, a.client_id, a.balance, a.created_at, c.id, c.name, c.email from accounts a inner join clients c on c.id = a.client_id where a.id = ?")
	if err != nil {
		return nil, err
	}
	defer stmt.Close()
